...
update :: proc() {
    ...
    if !game_board.stock_exhausted && len(game_board.stock.cards) == 0 {
        ...
        game_board.stock.texture = rl.LoadTexture("assets/stock_stackable.png")
    }
...
get_clicked_cards :: proc(game_board: ^Game_Board) -> ([dynamic]Card, ^Pile, bool) {
    clicked_cards: [dynamic]Card
    cards_are_moving: bool

    clicked_cards, cards_are_moving = find_clicked_cards(&game_board.waste.cards)
    if cards_are_moving {
         return clicked_cards, &game_board.waste, cards_are_moving
    }
    for i in 0..<len(game_board.tableau) {
        clicked_cards, cards_are_moving = find_clicked_cards(&game_board.tableau[i].cards)
        if cards_are_moving {
            return clicked_cards, &game_board.tableau[i], cards_are_moving
        }
    }
    if (game_board.stock_exhausted) {
        clicked_cards, cards_are_moving = find_clicked_cards(&game_board.stock.cards)
        if cards_are_moving {
             return clicked_cards, &game_board.stock, cards_are_moving
        }
    }
...
setup_game_board :: proc(game_board: ^Game_Board) {
    ...
    stock := Pile {
       texture = rl.LoadTexture("assets/card_back.png"),
    ...
    waste := Pile {
       texture = rl.LoadTexture("assets/card_empty.png"),
...
add_card_pile :: proc(pile: ^Pile, texture_name: string) {
    absolute_texture_file := strings.concatenate({"assets/", texture_name})
